...
export default function ProfilePage() {
  ...
  async function saveProfile(e: FormEvent) {
    ...
    setSavedMsg(null);
    try {
      await api.put("/users/me", { full_name: fullName, phone, guardian_name: guardianName, guardian_phone: guardianPhone, address });
      await refreshUser();
  ...
  return (
    <div>
      <PageHeader title="My profile" subtitle={`${user.student_profile?.class_name ?? ""} \u00b7 ${user.student_profile?.student_code ?? ""}`} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <form onSubmit={saveProfile} className="mt-4 space-y-4">
            ...
            <Field label="Guardian name"><input value={guardianName} onChange={(e) => setGuardianName(e.target.value)} className="fld" /></Field>
            <Field label="Guardian phone"><input value={guardianPhone} onChange={(e) => setGuardianPhone(e.target.value)} className="fld" /></Field>
            <Field label="Address"><textarea value={address} onChange={(e) => setAddress(e.target.value)} className="fld" rows={2} /></Field>
